...
export async function evaluateInitiative(ctx) {
...
export async function lockInitiativeJudging(ctx) {
  return pipeline(ctx, withAudit(async (innerCtx) => {
...
export async function approveReward(ctx) {
  return pipeline(ctx, withAudit(async (innerCtx) => {
    const result = rewardInitiative(innerCtx.user, innerCtx.initiativeId, innerCtx.totalReward, innerCtx.contributors);
    innerCtx.action = result.action;
