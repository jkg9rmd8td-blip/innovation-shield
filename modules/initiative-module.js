...
export async function moveInitiativeStageFlow(ctx) {
  return pipeline(
    ctx,
    withAudit(async (innerCtx) => {
      const result = moveInitiativeStage(innerCtx.user, innerCtx.initiativeId, innerCtx.stageKey, innerCtx.note);
      innerCtx.action = result.action;
      innerCtx.operation = "move_journey_stage";
      innerCtx.before = result.before;
      innerCtx.after = result.after;
      innerCtx.entityId = innerCtx.initiativeId;
      return result;
